...
class ROBINSONSParser(BaseParser):
    """Parser for ROBINSONS (Robinsons Bank) transaction files"""
    ...
    def parse_file(self, file_content: str) -> Dict[str, Any]:
        """Parse ROBINSONS transaction file"""
        try:
            ...
            logger.info(f"Processing ROBINSONS file with {len(lines)} lines")
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                # For ROBINSONS, split by both pipe and caret
                fields = self._split_line(line)
                
                # Validate minimum fields
                if len(fields) < self.min_fields:
                    logger.warning(f"ROBINSONS line {line_num} has insufficient fields: {len(fields)} < {self.min_fields}")
                    continue
            ...
            logger.info(f"ROBINSONS processing completed. Found {len(grouped_data)} ATM references, total amount: {total_amount}")
            ...
            logger.error(f"Error parsing ROBINSONS file: {str(e)}")
            raise
    
    def _split_line(self, line: str) -> List[str]:
        """Split line by both pipe and caret separators"""
        try:
            # Split by pipe first, then by caret
            fields = []
            parts = line.split('|')
            for part in parts:
                fields.extend(part.split('^'))
            # Clean and filter empty fields
            fields = [f.strip() for f in fields if f.strip()]
            return fields
        except Exception as e:
            logger.error(f"Error splitting ROBINSONS line: {str(e)}")
            return []
    
    def _parse_transaction_line(self, line: str, fields: List[str], line_num: int) -> Dict[str, Any]:
        """Parse individual ROBINSONS transaction line"""
        try:
            # Validate minimum fields
            if len(fields) < self.min_fields:
                logger.warning(f"ROBINSONS line {line_num} has insufficient fields: {len(fields)} < {self.min_fields}")
            ...
            if len(atm_ref_field) < 4:
                logger.warning(f"ROBINSONS line {line_num}: Invalid ATM reference format: {atm_ref_field}")
                return None
            
            atm_ref = atm_ref_field[:4]  # First 4 digits for grouping
            full_atm_ref = atm_ref_field  # Full reference
            ...
            logger.error(f"Error parsing ROBINSONS line {line_num}: {str(e)}")
            return None
    
    def _extract_amount(self, fields: List[str], line_num: int) -> float:
        """Extract amount from ROBINSONS fields (field 7, index 6)"""
        try:
            if len(fields) > 6:
                amount_str = fields[6].strip()
                amount = float(amount_str)
                logger.debug(f"Found ROBINSONS amount: {amount} from {fields[6]}")
                return amount
        except (ValueError, IndexError) as e:
            logger.warning(f"Could not extract ROBINSONS amount from line {line_num}: {str(e)}")
        
        return 0.0
    
    def _extract_date(self, fields: List[str], line_num: int) -> str:
        """Extract date from ROBINSONS fields (field 1, index 0)"""
        try:
            if len(fields) > 0:
                ...
                if date_str:
                    ...
                    logger.debug(f"Found ROBINSONS date: {date_str}")
                    return date_str
        except (ValueError, IndexError) as e:
            logger.warning(f"Could not extract ROBINSONS date from line {line_num}: {str(e)}")
        
        return 'N/A'
    
    def _is_valid_line(self, line: str) -> bool:
        """Check if line matches ROBINSONS format"""
        try:
            # Check for both pipe and caret separators
            if '|' not in line or '^' not in line:
                return False
            
            # Split line and check minimum fields
            fields = self._split_line(line)
            if len(fields) < self.min_fields:
                return False
    ...
    def validate_file_format(self, file_content: str) -> bool:
        """Validate if file matches ROBINSONS format"""
    ...
    def generate_formatted_report_line(self, transaction: Dict[str, Any]) -> str:
        """Generate formatted line for ROBINSONS report"""
        try:
            # For ROBINSONS, we return the original line as it's already in the correct format
            return f"{transaction['raw_text']}\n"
            
        except Exception as e:
            logger.error(f"Error formatting ROBINSONS report line: {str(e)}")
            return f"{transaction['raw_text']}\n"
